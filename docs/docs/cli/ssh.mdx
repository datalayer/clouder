...
clouder ssh my-server --user ubuntu --key my-custom-key

# Custom port
clouder ssh my-server -p 2222
```
...
clouder ssh my-server --key my-server
```

:::tip
When you create a VM with `clouder vm create` and choose "Generate new key pair", the key is named `<vm-name>-key`. The `clouder ssh` command automatically finds and uses this key.
:::
