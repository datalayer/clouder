...
```bash
clouder azure helm-values --cluster my-cluster -o azure-operator-values.json
```

If `--output` is omitted, the file is created at `~/.clouder/<cluster>/azure-operator-values.json`, which is the default path consumed by `plane/datalayer_plane/sbin/up.sh` for `datalayer_operator`.
