...
Select a kubeadm cluster as the default for Clouder commands and print the
kubeconfig export needed for local `kubectl` usage.
...
- The selected cluster is persisted as the default shown by `clouder kubeadm ls` and used when a later kubeadm command omits `<name>`.
...
The command uses the VM inventory persisted in
`~/.clouder/kubeadm/<name>/kubeadm.json`. It falls back to cloud discovery when
that inventory is incomplete, so normal information lookup does not require
active credentials for the cluster's cloud account.
...
`clouder kubeadm setup --node-mounts` installs the
[Datalayer Node Mounts](/services/system/node-mounts/) DaemonSet — the
[Local CSI driver](/services/system/local-csi/) (`local.csi.datalayer.io`),
and with `--node-mounts-gateway` the
[Node Mount Gateway](/services/system/node-mount-gateway/) agent as well — in
Step 7, right after the cloud storage provider. It is off by default: only
clusters serving Local Mounts or gateway mounts need a privileged DaemonSet on every runtimes
node.
...
| Option | Purpose |
| --- | --- |
| `--node-mounts` / `--no-node-mounts` | Install the driver (default: off) |
...
| `--node-mounts-image` | Driver image (default `datalayer/node-mounts:0.1.0`) |
| `--node-mounts-chart` | Chart path (default: `PLANE_HOME`, `DATALAYER_HOME`, or the source tree) |
