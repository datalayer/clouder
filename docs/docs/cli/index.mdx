...
| [`clouder node-mounts`](./kubeadm#local-csi-driver) | Check the Local CSI driver (`local.csi.datalayer.io`) installed with `clouder kubeadm setup --node-mounts` |
| [`clouder vm`](./vm) | Manage virtual machines (multi-cloud: Azure, AWS, OVH) |
| [`clouder ssh`](./ssh) | SSH into a virtual machine by name |
| [`clouder ssh-key`](./ssh-key) | Manage SSH keys (local plus AWS/OVH cloud key registries) |
| [`clouder s3`](./s3) | Manage S3 buckets (AWS and OVH contexts) |
...
See the full workflow in
[`clouder kubeadm`](./kubeadm#clouder-kubeadm-set-default).
