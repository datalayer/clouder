...
```bash
eval $(minikube docker-env)
cd $DATALAYER_HOME/etc/docker/jupyterpool && \
  make build && \
  make push
plane up minikube jupyterpool
plane ls
# open $(minikube service jupyterpool-jupyterpool-svc --namespace datalayer --url)/api/jupyterpool
