...
kubectl get pv | grep datalayer-jupyter
...
3. Create a Shared Filesystem PVC for the user data in the `datalayer-jupyter` namespace with the following command.
...
flowchart TB
    ...
    subgraph "node (datalayer-jupyter)"
    subgraph pod
    rk[Remote Kernels]
