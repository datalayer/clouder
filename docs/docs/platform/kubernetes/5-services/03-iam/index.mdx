...
```mermaid
sequenceDiagram title Create a Jupyter Kernel from JupyterLab
    JupyterLab->>Jupyter Service: Request a Jupyter Kernel
    ...
    Jupyter Service->>Operator: Request a Kernel
    Operator->>Operator: Authn with Secret Key
    Operator->>Jupyter Kernel: Create a Kernel Pod and Ingress
...
```mermaid
sequenceDiagram title Access a Jupyter Kernel from JupyterLab
    JupyterLab->>Jupyter Kernel: Access a Jupyter Kernel (URL + Token)
    Jupyter Kernel-->>Jupyter Kernel: Check Jupyter Token
    break if token invalid
      Jupyter Kernel-->>JupyterLab: Invalid
    end
    Jupyter Kernel-->>Jupyter Kernel: Execute request
    Jupyter Kernel-->>JupyterLab: Response
```
