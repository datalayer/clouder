...
2. The private Datalayer images for e.g. [IAM](/platform/kubernetes/5-services/03-iam/index.mdx), [Jupyter](/platform/kubernetes/5-services/04-jupyter/index.mdx), [Operator](/platform/kubernetes/5-services/02-operator/index.mdx)... services.
...
Your Kubernetes cluster need to pull those Docker Images, therefor you need to create a Registry Secret in each of the [Namespace](/platform/kubernetes/cluster/namespaces/).
...
```bash
for ns in datalayer-api datalayer-solr datalayer-system datalayer-jupyter datalayer-growth datalayer-router datalayer-traefik datalayer-nginx kube-system
do
