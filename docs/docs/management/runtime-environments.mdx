...
User Environments, which accounts and organizations define, version and build themselves, are on the [Environments](/services/environments/) page, with what of them runs today.
...
`datalayer-buildkit` is one replica with `strategy: Recreate` — a build's own layer cache lives on the pod that made it, so a rolling update would run two builders with two half-warm caches for no benefit over replacing the one there is (the chart's own README says so). The cost of that: restarting it — `plane up datalayer-buildkit` on a changed chart, or a node drain that evicts it — kills whatever build is running mid-solve, the same as killing any other single-replica stateful daemon. Before restarting it on purpose:
...
An empty list means nothing is mid-flight. A non-empty one names builds still running — wait for them to reach a terminal status (`ready`, `partially_ready` or `failed`), or accept that each one in flight ends `failed` with `DL_ENV_PROVIDER_ERROR` (retryable) the moment the daemon it was talking to disappears; a retry after the roll starts over from `resolving`, since nothing about a killed solve is resumable. There is no drain signal the daemon itself understands — this check, run by hand before the deploy, is the whole mechanism.
