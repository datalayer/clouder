...
The registry of Datalayer Environments is a separate root, `terraform/environments-registry`, and is not deployed with this stack's Makefile: `clouder aws ecr-environments deploy` runs it end to end. See [AWS ECR for Environments](../../cluster/2-registries/3-aws-ecr-environments/index.mdx).
...
1. System services: cert-manager, ingress (Traefik), Solr, OTEL, observer, Vault, Kafka, Pulsar, OpenFGA, Datashim ([deprecated](/services/system/datashim/)), mailer.
