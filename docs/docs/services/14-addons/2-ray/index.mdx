...
1. `GET /clusters?namespace=<ns>`
2. `POST /clusters`
3. `GET /clusters/{name}?namespace=<ns>`
4. `DELETE /clusters/{name}?namespace=<ns>`

### RayJob Management

1. `POST /clusters/{cluster_name}/jobs`
2. `GET /jobs?namespace=<ns>&cluster_name=<optional>`
3. `GET /jobs/{name}?namespace=<ns>`
4. `DELETE /jobs/{name}?namespace=<ns>`
5. `GET /jobs/{name}/logs?namespace=<ns>&pod_name=<optional>&container=<optional>&tail_lines=200`
6. `GET /jobs/{name}/events?namespace=<ns>&limit=100`
...
3. CLI command group:
   - `datalayer ray clusters list|get|create|delete`
   - `datalayer ray jobs list|submit|status|delete|logs|events`
...
The chart creates a `ClusterRole` and `ClusterRoleBinding` allowing `rayclusters` and `rayjobs` CRUD across namespaces.
