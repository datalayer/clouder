...
Proven live on r1, 2026-09-14, against the real daemon and the real registry: a solve (164.20 s cold), a build that pushed a 2.7 GB image, a retried build with no collision, and `exists` and `delete`. Through the real Runtimes API and `EnvironmentBuildWorkflow`, a build now reaches the full `attest` step: it scans clean against the patched base channel, signs with the KMS key, and a `cosign verify` run by hand against the pushed signature exits `0`. What stops every build afterward is not attest — it is `smoke_test`'s own missing internal trial route (E1-14), the one piece left before a version can reach `ready`. Not yet proven: the resolver's p95 under 45 s over 20 runs, and the Operator's own verify-before-launch gate running as a service against a real digest on r1 (it needs a pod actually launched from one, which needs that same missing route).
