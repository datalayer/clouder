...
`POST /evalsets/{id}/clone` `{name?}` takes a benchmark into the account the
request names (`datalayer_solr.evals.clone_eval`). The result is a new, private
evalset at version 1:
...
The clone writes a `fork` derivation into the Library (see the Library page).
