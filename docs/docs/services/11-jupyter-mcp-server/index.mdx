...
two replicas. The `mcp-*` collections must already exist (`plane solr-init`,
...
**On prod1 it did not launch anything until 2026-09-05, and half of it still
does not.** The gateway's own Runtimes client sent `environment_name`; the
operator's `CreateRuntimeRequest` takes `environment: {name}` and requires a
`credits_limit`, so every first execution was refused `422` — a
`runtimes_unavailable` to the agent, which reads as an outage. The
environment is sent as the operator takes it now; the credits limit is not
sent yet. The worker's own launcher computes it as the environment's
`burning_rate × 60 × minutes`, and the gateway's launch has to do the same
before `execute_code` on a fresh session works on a deployment. Until then
`launch_sandbox` with an explicit environment — the worker's path — is what
works, and what the walks use. `use_sandbox` and
