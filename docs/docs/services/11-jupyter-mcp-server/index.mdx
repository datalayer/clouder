...
:::caution Still open: `list_sandboxes` answers for the worker, not the session

A session whose sandbox the *gateway* launched is told by `list_sandboxes`
that it has none, while `launch_sandbox` on the same session answers that it
already has one. The tool lists the worker's own registry; the binding store
is what the session actually holds.
