...
`server/discover` is where to look: `known` says what the deployment's
vocabulary defines and `capabilities` says what this session's sandbox
reported. A tool missing while its capability is in `known` and not in
`capabilities` is this, and not a broken gateway.
