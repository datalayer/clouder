...
against a pod that was asked what it was running first. One edit currently
arrives as two notifications — the publishing wrapper and the watcher seeing
the same change — which costs a subscriber one extra refetch.

What is still open is which *cell* changed: the notification names the
notebook, and an agent is told to refetch rather than told what moved.
...
One thing that follows is **not** done: a worker is per *user*, so its spans
are filed under the person even when the gateway files that call under an
organization. For a personal call the two agree; for an organization's they
do not, and making them agree needs the account per request rather than per
worker.
