...
:::danger Sharing is refused on prod1 until the Runtimes plane is deployed

`PUT /runtimes/{name}/sharing` answers `500 Database Request Error` for a
user, a team, an organization and an agent alike. Runtimes writes the
runtime's expiry straight into a Solr **date** field, and the operator
answers epoch seconds as a string — `'1788671641.000'`. A date field that is
not a date makes Solr refuse the whole document, which is why the principal
kind made no difference: the field at fault was the one that never varied.

Fixed in `datalayer_solr.runtime_sharing`, and it ships in the **Runtimes**
image on the r1 plane. Until that is deployed, neither the tools nor the
console's Share button can share a sandbox.

Every test of `to_document` called it without an expiry, which is how the
value that reaches Solr in production never reached it in a test.
...
:::caution A grantee cannot yet reach the runtime by name

`GET /runtimes/{name}` answers `404` for a grantee, so the worker's
attach-by-name cannot find a runtime shared with it and `use_sandbox` answers
*"No sandbox with id …"*. The permissions route answers a grantee perfectly
well; the runtime route does not. Until it does, a share can be made and not
used.
