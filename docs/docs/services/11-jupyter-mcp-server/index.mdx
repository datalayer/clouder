...
| `mcp.refusals` | counter | `reason` — `access_check_failed`, `access_revoked`, `attribution_failed`, `capability_missing`, `environment_capability`, `header_body_mismatch`, `item`, `policy_unavailable`, `relaunch_from_snapshot`, `runtimes_unavailable`, `sandbox_lost`, `scope`, `transport_direct`, `unknown_handle`, `policy:…`, `rate_limited:…`, `quota:…`; `usage_account_uid` when the call was made in an organization | Calls refused before they reached a worker |
