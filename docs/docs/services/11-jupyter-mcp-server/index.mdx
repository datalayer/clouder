...
**Losing a pod under load behaves.** Delete one of two while 30 agents work:
9 failures out of 581, all connection errors, and the same thirteen active
handles before and after. That is the pod's in-flight requests and nothing
else, which is the claim.
...
:::caution The autoscaler is not enabled on prod1

The table above describes the chart's default. On prod1 there is no
`HorizontalPodAutoscaler` at all and the Deployment carries an explicit
`replicas: 2`, so the scale-up and scale-down behaviour documented above is
not what is running. Check with `kubectl get hpa -n datalayer-api` before
relying on it.
