...
:::note Building the image needs `agent-runtimes` 1.2 first

`agent-runtimes 1.1.0` pins `opentelemetry-semantic-conventions<0.62`, and
`opentelemetry-sdk 1.44.0` requires exactly `0.65b0`. So the OpenTelemetry
1.40 → 1.44 bump cannot be installed beside it and the image will not build
with both.

Worth knowing because of *how* it fails: pip reports a conflict about
`mcp[cli]` and `fastmcp-slim`, which is where it gave up rather than what is
wrong. Resolve the two packages in a clean environment and they install
together fine; add the OpenTelemetry constraint and the real conflict names
itself.
...
### A deploy keeps the sandbox and empties it
...
**The kernel is.** A variable set before the rollout is gone after it, and
the call that finds out answers `NameError` — as though the agent had never
assigned it. Counted from inside the runtime there is one kernel before and
one after, and it is a different one: the kernel holding the session's state
is gone by the time the new worker looks, and the worker reuses whatever it
finds instead.

So a routine deploy silently empties every session's memory, and an agent
learns it from a name that is suddenly undefined rather than from anything
that says the state was lost. The mechanism is not settled: the kernel client
already declines to shut down a kernel it did not start, and which of the
runtime, the culler or the drain removes the old one has not been
determined.

Until it is, treat a deploy the way you would treat a lost sandbox: what was
in memory is gone, and what was written to the notebook is not.
