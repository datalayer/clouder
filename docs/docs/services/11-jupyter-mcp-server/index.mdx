...
Borrowed toolset tools (below) are the exception nobody can fix here: a
source's manifest carries a description and an input schema and no hints,
so the cautious defaults answer for them until Contents lets a source say.
