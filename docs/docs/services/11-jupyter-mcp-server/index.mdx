...
:::info The cap is checked at the reservation too
The gateway checks it before a launch, and that is not the only door: a
...
IAM applies it when the reservation is made. Only to **agent** reservations,
identified by the `client_id_s`/`agent_uid_s` dimension: the cap is a rule
about agents, and refusing a person launching their own runtime would refuse
somebody under a rule that was never about them. Only the **organization's**
layer, since the count is per billing entity — a team's number against an
entity-wide count would refuse a team holding nothing, and the gateway is
where the team cap belongs.

A refusal there is a `409` naming both numbers and who can raise the limit.
:::
