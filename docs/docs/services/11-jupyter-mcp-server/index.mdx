...
Only sandboxes on providers that declare the `snapshot` capability are
...
capability. An agent on Modal never sees it.
...
| `mcp.limit.unenforced` | counter | `limit` — `maxCreditsPerDay`, `maxConcurrentSandboxes`; `reason` — `unreadable`, `incomplete` | A limit **not** applied because the figure it compares against could not be read |
