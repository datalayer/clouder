...
| `mcp.refusals` | counter | `reason` — `access_check_failed`, `attribution_failed`, `capability_missing`, `environment_capability`, `header_body_mismatch`, `item`, `policy_unavailable`, `runtimes_unavailable`, `sandbox_lost`, `scope`, `unknown_handle`, `policy:…`, `rate_limited:…`, `quota:…` | Calls refused before they reached a worker |
