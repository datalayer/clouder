...
**It still does not steer.** Both ends are confirmed on the running pods and
the worker logs that it will follow the sandbox, and the follower's own line
never appears. What remains is to find whether the header reaches the
worker's scope at all.

Until it does, treat a deploy the way you would treat a lost sandbox: what
was in memory is gone, and what was written to the notebook is not.
