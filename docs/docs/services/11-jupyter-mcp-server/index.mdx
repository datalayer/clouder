...
Two things that follow are **not** done, and both are visible from the
outside. The worker's spans start their own trace: the gateway injects
`traceparent` into every forward, and the worker's ASGI app is not
instrumented, so nothing extracts it. And a worker is per *user*, so its
spans are filed under the person even when the gateway files that call under
an organization — for a personal call the two agree, for an organization's
they do not.
