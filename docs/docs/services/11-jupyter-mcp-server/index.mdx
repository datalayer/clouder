...
- the MCP transport on `/mcp`
- the RFC 9728 metadata
- the `/api/mcp/*` REST routes
- scope and item enforcement
- the rate limiter
...
The **Auth** column is the boundary on who may call a route at all; **Notes**
is what it returns or how it behaves once a caller is let through.
...
**On audit forwarding.** Forwarding **never fails the call it describes** —
holding up a tool call because a customer's SIEM is down would be the worse
trade — so a failure is invisible unless something reports it, and a silently
dropped audit record looks exactly like nothing having happened.
...
| `jupyterMcpServer.autoscaling` | **`enabled: false`**, `minReplicas: 2`, `maxReplicas: 8`, `targetCPUUtilizationPercentage: 70` | Off by default; see below |
...
**On `jupyterMcpServer.autoscaling`.** The chart **refuses to render** if it
...
`attribution_failed`, `capability_missing`, `environment_capability`,
`grant_revoked`, `header_body_mismatch`, `item`, `policy_unavailable`,
