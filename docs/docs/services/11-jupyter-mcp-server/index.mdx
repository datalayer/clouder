...
The cause is one line of `DatalayerTaskStore`:

```python
await asyncio.to_thread(work, self._dao())
```

Python evaluates an argument before the call it belongs to, so `_dao()` —
which opens a Solr connection through ZooKeeper on first use — ran on the
event loop. The line whose whole purpose was to get this off the loop kept
the expensive half on it. Both call sites build the repository inside the
thread now, under a `threading` lock rather than an `asyncio` one, because
there is no loop where `_dao` runs.

Worth keeping for the shape rather than the line: **a synchronous call on a
worker's loop takes the whole worker down, not the request that made it**,
and the directory cannot tell. If a worker ever goes quiet again, the first
thing to read is `/proc/<pid>/task/*/wchan` in its pod — a main thread that
is not in `ep_poll` is a blocking call that should have been in a thread.
