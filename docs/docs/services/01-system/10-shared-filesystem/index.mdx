...
| --- | --- |
| Runtime Pods (Operator) | Whole claim in the companion container at `DATALAYER_SHARED_FS_MOUNT_PATH`. The runtime container gets its folders one of two ways: from the gateway when it is on, or — when it is off — one `subPath` per home folder, at `/home/jovyan/<handle>`. |
