...
| `datalayer-contents` | Whole claim, at the same path. |
| [Node Mount Gateway](/services/system/node-mount-gateway/) | Whole claim, **once per node**, in the `datalayer-node-mounts` DaemonSet, at `nodeMountGateway.sharedFilesystemMountPath`. |
...
- **`datalayer-contents`:** serves the Home Folder browser from the same bytes the sandbox sees.
