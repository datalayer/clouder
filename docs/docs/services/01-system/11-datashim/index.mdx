...
<Stack direction="horizontal" alignItems="flex-start" padding="none" style={{marginBottom: 30}}>
  <Label color="green">In use</Label>
  <Label color="yellow">To be replaced</Label>
</Stack>
...
[Node Mount Gateway](/services/system/node-mount-gateway/), which binds a
...
Nothing has moved yet, and nothing will until the gateway's bucket path has
been proven against a real bucket. Until then this page is current and
Datashim stays exactly as it is. The comparison, and what retiring it buys, is
on the [gateway's page](/services/system/node-mount-gateway/#datashim-and-what-replaces-it).
