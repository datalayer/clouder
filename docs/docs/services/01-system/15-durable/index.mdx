...
Authenticated with `DATALAYER_DURABLE_API_KEY` in `X-API-Key`, the header
every Datalayer service proves itself with. **An unset key refuses every
caller** rather than admitting all of them — a workflow API open to the
cluster network is a way to start work as anybody.
...
| `DATALAYER_DURABLE_API_KEY` | What a caller proves itself with. Empty means nobody may call, which is a refusal and not a wildcard |
...
`check_configuration()` runs at startup and refuses on a setting that would
fail later and somewhere else — an unknown engine, or no queues at all.
