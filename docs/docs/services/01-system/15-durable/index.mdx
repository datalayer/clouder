...
:::warning Only the gateway's key is in the chart today
`datalayer-durable`'s Deployment injects
`DATALAYER_JUPYTER_MCP_SERVER_API_KEY` and not the Contents or Operator ones.
Both of those callers therefore get `401` in a deployment, silently as far as
this service is concerned — Contents cannot run its operations here and the
Operator cannot read them. Add them to the release secret when those paths
are wanted.

The chart also injects `DATALAYER_DURABLE_API_KEY`, which **nothing reads**.
It is declared in `config.py` with a docstring saying it is what the gateway
calls with, and that has never been true: the gateway calls with its own key
against this service's audience. Setting it changes nothing.
:::
