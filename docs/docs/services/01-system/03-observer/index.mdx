...
**🚨 RETIRED** — The Datalayer Observer is deprecated and will be removed in a future release. It is replaced by the [Datalayer OTEL](/services/system/otel) service that only deploys the OpenTelemetry Collector and relies on the services to be instrumented and to send their telemetry to it.
