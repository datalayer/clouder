...
Enabled on 2026-09-09, the way the memory cluster's is. Set
`DATALAYER_POSTGRESQL_BACKUP_S3_BUCKET_NAME` and the `aws-creds` secret in the
`datalayer-dbos` namespace, and `plane postgresql-dbos-create` writes the
`backup:` stanza into the cluster — the same shared bucket and `30d` retention
as every other Datalayer cluster. Then:
