...
| `OrchestrationWorkflow` | `orchestration` | `mint_execution_credential` → `resolve_worker` → `authorize_context` → `claim_dispatch` → `dispatch` → `await_acceptance` → `monitor` → `checkpoint` → `reconcile` → `commit_artifacts` → `release` → `project_execution` | One execution the orchestration control plane in ai-agents accepted, keyed on the execution id with its account and person as arguments. The A2A and ACP adapters of `agent-runtimes` run in this worker, and the execution store (the `orchestration` Solr collection, opened for the execution's account) is the source of truth: every observation is recorded there through the canonical lifecycle, and every command the control plane received — a cancel, a steer — is read from there while the worker works, a signal only waking the run. `claim_dispatch` records the attempt and the process that claimed it before anything is sent; `dispatch` sends the objective only from that process, re-attaches through the recorded protocol handle from any other, and with no handle recorded says the delivery is unknown rather than sending twice. Context references are held against a resolver as the person, and a required one nobody can vouch for refuses the execution (`context_unavailable`). Artifacts are committed by the first attempt to commit. A failure the engine gives up on is recorded on the execution with its code. Heartbeats every 30 s |
...
single key for the service: four identities are known, each with the least
...
All four are on the Deployment as **optional** secret references, so listing
