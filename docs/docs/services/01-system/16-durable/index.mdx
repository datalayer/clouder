...
**Versioning is a build id.** Every worker reports the build it runs
(`DATALAYER_DURABLE_APPLICATION_VERSION`, the chart's `appVersion`) and
Temporal keeps a run on a compatible worker. A worker that reported nothing
would take a run started by code it does not have. A change to a workflow's
*shape* — steps added, reordered, removed — needs `workflow.patched()` as
well, so a run started before the change finishes the way it started; there
is no shape change yet to guard, and the first one must add it.
...
```bash
docker compose -f docker-compose.local.yml --profile temporal up -d
...
It removes `dbos_durable` and `dbos_agents` and everything in them: every
