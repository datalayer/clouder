...
### Drilled with a real benchmark, on every engine (2026-09-12)

The fake proves the catalog is walked and replayed correctly, and nothing
more: it is not itself a DBOS runtime or a Temporal server, so a restriction
that belongs to one of those has nothing in the fake to violate. `none`,
`dbos` and `temporal` were each drilled with a real benchmark launch, over
`plane local` with `DATALAYER_DURABLE_ENGINE` switched between the three —
and two real, engine-specific defects turned up, one per real engine, that
no amount of testing against the fake could have found.

**`dbos`: a step cannot signal a sibling workflow.** `release_pool` — a
step, not a workflow — tells each slot's sandbox its life is over by
calling the engine's `signal`. DBOS's `send` refuses to run inside a step:
its assertion wants either a workflow's own top level or nothing
DBOS-tracked at all, and a step is neither — `cur_ctx.is_within_workflow()`
is true but `is_workflow()` is false. The service's own signal tests call
`signal` from plain test code, which has no DBOS context at all: that is
what an HTTP route answering a real approval looks like, and it was never
broken, which is also why it never caught this. Fixed by clearing DBOS's
local context for the call (`_outside_any_workflow` in `dbos_engine.py`),
so DBOS takes the path it already has for exactly this shape of caller:
routing the send through its own throwaway workflow.

**`temporal`: a long step reads as a dead worker.** `run_cases` runs every
task of a slot concurrently and can take minutes, calling back into
Temporal for none of it. Every step carries a `heartbeat_timeout` so a
worker that dies mid-step is noticed quickly — but without a heartbeat
*during* the step, that same timeout reads the step that is working as the
one that died: `run_cases` was heartbeat-timed-out and reported failed a
few dozen seconds into every real run, measured live. `step_activity` now
ticks `activity.heartbeat()` on a background loop at a third of the
configured timeout for as long as the step runs, cancelled the moment it
finishes either way. `temporalio.testing.ActivityEnvironment` — no server —
is what let this be caught and regression-tested without the real stack.

Shipped in durable `0.1.17`. The `dbos` fix is live on prod1, which runs
that engine; the `temporal` fix is dormant there until Temporal is the
active engine (see above).
...
**One `datalayer-durable`, on the runtimes plane** — not one per plane. `plane
up datalayer-durable` (`datalayer_durable()` in `up.sh`) is a single release,
placed beside the runtimes whose sessions its workflows execute in; nothing in
it branches on which cluster happens to be current. Whichever cluster that is
today, that is the one durable install that matters — never a peer copy for
every plane that happens to call it.

**Every caller reaches it the same way, wherever the caller itself runs**: at
its public URL, `DATALAYER_DURABLE_URL`, through the real ingress `plane reup
datalayer-durable` renders at `DATALAYER_DURABLE_HOST` — never the in-cluster
name `datalayer-durable-svc.datalayer-durable`, which only resolves for a
caller already inside durable's own cluster, and a name that resolves today
works right up until the caller moves. See
[Reaching it from the API plane](#reaching-it-from-the-api-plane) for what
that takes.

:::caution A second install is a bug, not a second thing to track
On 2026-09-12 `datalayer-durable` existed on **both** prod1 and r1 — not by
design, but because a rollout run against the wrong `kubectl` context does
exactly this. The two had pulled `durable:0.1.7` three days apart and run
different code under one tag: prod1's copy had five workflows and no
`ai-agents` caller identity, so every `ai-agents` call that happened to reach
it answered `401`. If `kubectl get pods -n datalayer-durable` answers on a
cluster that is not today's runtimes plane, that is a leftover from a wrong
context, not a second install to keep in sync — tear it down. Bump the image
tag on every real build regardless, so two builds are never mistaken for one
even when there is properly only one install.
:::

**As of 2026-09-13, `datalayer-durable`, `ai-agents`, the gateway
(`jupyter-mcp-server`) and the `scheduler` all run on r1, not prod1** — a real
redeployment this session, not a renaming. Elsewhere on this page and its
neighbours, "prod1" and "the API plane" for one of those four now means
wherever that service actually answers — check with `kubectl` against the
plane in question rather than assume from a cluster's name. The mechanism
above is unchanged by the move: a caller reaches durable at its public URL
through the real ingress whether or not it happens to share durable's own
cluster today, because that placement is not guaranteed to survive the next
migration either.
...
and has nothing to do with durable execution.
...
| `project_run` | **Solr directly**, the `evals` collection — and ai-agents, as the person, for what the run leaves in their inbox: how it ended (`run.completed`, `run.blocked`, `budget.reached`) and the tasks its scorer could not decide (`task.review`, one notification per run) |
...
:::warning r1's chart kept the in-cluster collector default, found live 2026-09-14
Only prod1 runs an in-cluster OTEL collector; r1's `datalayer-otel` namespace
is empty. Runtimes and the Operator on r1 already point at prod1's public
OTLP route (`https://prod1.datalayer.run/api/otel/v1/otlp/v1/…`,
`http/protobuf`), but durable's chart still carried the gRPC in-cluster
default, `datalayer-otel-otel-collector-svc…:4317`, which resolves to
nothing on r1 — every export failed with `UNAVAILABLE`, silently, since a
failed export is not why a workflow step fails. `up.sh` now gives durable
the same four `OTEL_EXPORTER_OTLP_*` settings Runtimes already had, matching
across services rather than left per-plane. Verified after the fix: durable's
build metrics (`environments.builds` and friends, see
[Environments](/services/environments/)) read back through the OTEL query
API.
:::
