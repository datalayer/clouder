...
| `usage-aws` | AWS Marketplace metering ledger and run heartbeats |
...
| `orchestration` | ai-agents | Orchestration executions — one document per execution, attempt, event, acknowledgement and artifact, told apart by `type_s` — and the idempotency keys delegate commands claim; read by the control plane and written by the durable worker |
...
### Runtimes Collections (12)

Collections specific to the runtimes plane, Contents and the Environments
registry included — the Contents service is deployed there, beside the NFS that
backs the Home Folder and the Volumes, and the registry belongs to Runtimes:
...
| `environments` | Environments: one document per owner and name, which its `id` makes unique |
| `environment-versions` | Environment versions, whose `id` is `<environment_uid>:<n>`, and the locks they resolve to, each stored whole under its digest |
| `environment-artifacts` | Built artifacts per version, variant and region, with the cache key and the retention state the collector scans |
| `environment-builds` | Environment builds, and their logs as chunks written once each |

The inventory is 19 Core plus 7 AI plus 12 Runtimes collections, 38 in total.
...
`content-operations` is the busiest of the four: within it live the durable
operation jobs, transfers (with their part rows), datasource queries, Dataserver
registrations and capability tickets, MCP sessions/calls/approvals, local-bridge
sessions, and the **synchronization** sessions with their conflict rows
(`content_sync_session` and `content_sync_conflict`). A synchronization's block
content itself is *not* in Solr — the changed blocks are staged in managed
object storage and composed into an object version there; only the session and
conflict state is a Solr document. All of these finished records age out under
`DATALAYER_CONTENTS_OPERATION_RETENTION_DAYS` (see the Contents service page),
and `contents_mappings.json` is the authoritative per-family record of each
one's `type_s`, fields and nested children.
...
#### Mapping 15: a Pod name is not a uid

Version 15 renames a sandbox binding's misnamed field. It stored the Pod
name Runtimes knows the sandbox by, under the name `sandbox_uid` — and a
sandbox's uid is a ULID, which that field never held. The binding stores
`runtime_name_s` now, and the uid is derived from the binding's own handle
rather than stored at all, because it is a function of it.

**Not additive.** A binding written under version 14 has no
`runtime_name_s`, so the gateway cannot reach that sandbox through
Runtimes. Bindings are short-lived — they carry `BINDING_TTL_SECONDS` — so
the old ones age out within the hour rather than being migrated. What a
deployment loses is the sessions in flight at the rollout, which a gateway
restart ends anyway.
...
A new collection takes its configset from the `datalayer-solr:9.10.0` image
the init pod runs, and a collection that exists keeps the configset it was
created with. So after a change under `plane/etc/dockerfiles/datalayer-solr/config`,
rebuild and push that image before creating the collections that need the
change, or they are created from the older schema. The environments
collections are the case in point: their locks live in the `*_stored` field,
which an image built before 2026-09-11 does not have. Check the image first:

```bash
(cd plane/etc/dockerfiles/datalayer-solr && make build-dev push)
docker run --rm --entrypoint grep $DATALAYER_DOCKER_REGISTRY/datalayer-solr:9.10.0 \
  -c 'name="\*_stored"' /opt/solr/config/managed-schema
# 1
```
...
#   2) Runtimes   - Runtimes collections (12 collections, Contents and Environments included)
#   3) All        - All collections (38 collections)
#   4) AI         - AI collections (7 collections: ai-agents, evals, orchestration,
...
for COLLECTION in ai-agents api-keys contacts credits evals events iam inbounds invites library notifications oauth-clients oauth-codes oauth-grants orchestration outbounds schedules secrets spaces success tool-approvals usage usage-aws mcp-gateway mcp-tasks mcp-audit ray runtime-registrations runtime-checkpoints sandbox-snapshots contents content-objects content-operations content-audit environments environment-versions environment-artifacts environment-builds
do
    echo
...
| File | Scope | CR Name |
|------|-------|---------|
| `datalayer-backup-s3.yaml` | All 38 collections | `datalayer-solr-collection-backups` |
| `datalayer-backup-s3-ai.yaml` | The 7 AI collections | `datalayer-solr-ai-backups` |
| `datalayer-backup-s3-core.yaml` | 19 Core collections | `datalayer-solr-collection-backups` |
| `datalayer-backup-s3-runtimes.yaml` | 12 Runtimes collections, the four Contents and four Environments ones included | `datalayer-solr-collection-backups` |
