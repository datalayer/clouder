...
The inventory is 18 Core plus 7 AI plus 12 Runtimes collections, 37 in total.
...
#   3) All        - All collections (37 collections)
...
for COLLECTION in ai-agents api-keys contacts credits evals events iam inbounds invites library notifications oauth-clients oauth-codes oauth-grants orchestration outbounds schedules secrets spaces success tool-approvals usage mcp-gateway mcp-tasks mcp-audit ray runtime-registrations runtime-checkpoints sandbox-snapshots contents content-objects content-operations content-audit environments environment-versions environment-artifacts environment-builds
do
    echo
...
| File | Scope | CR Name |
|------|-------|---------|
| `datalayer-backup-s3.yaml` | All 37 collections | `datalayer-solr-collection-backups` |
