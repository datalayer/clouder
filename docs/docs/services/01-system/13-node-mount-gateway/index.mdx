...
the companion's link, under `/home/jovyan`. For a Home Folder or a share it
...
attachment's `mount_path`. An attachment at `/home/jovyan` itself is the Home
...
`/home/jovyan/<name>` is a **share** — a `files` folder attached by claim
...
than bound twice at one place; one outside `/home/jovyan` is refused with
...
and that is right. Here root mounts and the sandbox's `jovyan` reads, in
...
purpose: a creation-time `subPath` would leave `/home/jovyan/<dir>` root-owned,
...
`/home/jovyan` and joins every content — the checkout, the share, the bucket —
