...
**A pooled Pod is the only one that gets them this way.** A spawned Pod already
carries the Environment's `git` and shared-filesystem contents in its spec, so
granting those again would mount each one twice. A pooled Pod was built before
anyone knew which Environment it would serve, which is why the grant is the only
route it has. Its buckets are the one exception: a spawned Pod no longer renders
an `s3` content at all (Datashim retired, audit 83), but no Environment both
carries a bucket and forces a spawn, so in practice a bucket is always a
pooled-Pod `cloud-storage` grant.
