...
| `GATEWAY_KIND_OFF` | Refused by the **Operator**, not the agent: a grant of a kind whose switch is off on this deployment. The Operator reads the same four switches (`DATALAYER_NODE_MOUNT_GATEWAY_CREDENTIALS`, `_BUCKETS`, `_LOCAL_BRIDGES`, `_REPOSITORIES`) so it never writes what the agent would answer `SECRET_REFUSED`; for an Environment's content it means the pool is declined and the launch spawns with the content. |
| `NODE_MOUNT_GATEWAY_INVALID_TARGET` | A target that is not a relative path of clean segments — one for a Home Folder or a share, `datasets/<name>` for an Environment's contents, three at most; never absolute, never walking |
