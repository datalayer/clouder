...
flowchart TB
  ...
  home["/home/datalayer/HANDLE<br/>symlink, made inside the sandbox"]
...
## Why `/home/datalayer` is not a volume either

The folders are bound at `/mnt/datalayer/{handle}` and reached at
`/home/datalayer/{handle}` through a **symlink made inside the sandbox**, by the
same kernel the companion already runs start hooks in.

The first design put the home directory on a volume shared with the companion, so
...
| --- | --- | --- |
| `runtime-pools.datalayer.io/node-mount-gateway-mounts` | Operator | The mount set the Pod should have |
| `runtime-pools.datalayer.io/node-mount-gateway-ready` | Gateway agent | What the agent applied |
| `runtime-pools.datalayer.io/node-mount-gateway` (label) | Operator | Whether the Pod carries the gateway volume |

- **`node-mount-gateway-mounts`** is the mount set:
  `{"hash": …, "mounts": [{"source": "home/users/UID", "target": "HANDLE", "mode": "rw", "allow_exec": true}]}`.
  `source` is relative to the claim; `target` is one path segment.
- **`node-mount-gateway-ready`** is what the agent applied:
  `{"hash": …, "state": "ready|degraded|failed", "mounted": [...], "failed": {...}}`.
  The Operator waits for the **hash it asked for** — an answer for the previous set is not an answer for this one.
- **`node-mount-gateway` (label)** is whether the Pod carries the gateway volume. A pool built before the gateway still holds Pods without it, and one of those must never be handed to a hot attach.
...
### A target is a path, one segment usually

A grant's `target` is where it appears under `/mnt/datalayer` and, through
the companion's link, under `/home/datalayer`. For a Home Folder or a share it
is one segment — `eric`, `eric-ro`. For an Environment's contents it is the
path the Environment declares beneath the home, `datasets/<name>` or
`models/<name>`, because that is where the manual says they are and where a
spawned pod already puts them. The rule was one segment only, and its cost
was invisible: an Environment whose contents sit two deep could not be
served from the pool at all, so every launch of it that mounted anything —
a Home Folder launch included — was given a cold pod, twenty seconds
instead of three. A target is now a relative path of clean segments, three
at most; the agent makes the parent directories in the pod's tree and takes
them away again when the leaf is revoked, its release sweep looks for mount
points rather than entries (a nested target's parent is a directory, not a
mount), and the companion links at the same depth, making `~/datasets` when
it has to and never overwriting a folder of the user's.
...
| --- | --- | --- |
| `files` (a Home Folder — yours, or one shared with you) | `mount` | `files`, from the `source_path` Contents puts on the prepare answer |
...
The first row has two cases, and the Operator tells them apart by the
attachment's `mount_path`. An attachment at `/home/datalayer` itself is the Home
Folder capability said as an attachment: it is rendered with the person's own
folders, one grant per membership, and never as a share. An attachment at
`/home/datalayer/<name>` is a **share** — a `files` folder attached by claim
path, which is how someone else's Home Folder, or your own read-only under a
second name, reaches a sandbox. The Operator builds that grant from the
`source_path` Contents puts on the attachment's *prepare* answer (and on the
manifest): source is that path, target is `<name>`, mode is the attachment's,
kind is `files`. A share whose `<name>` is already a mount is skipped rather
than bound twice at one place; one outside `/home/datalayer` is refused with
`FILES_PATH_UNSUPPORTED`; a walking source is refused where the grant is
written, with `INVALID_SOURCE`. An attachment with no `source_path` is what a
Contents older than this page sends, and is left alone — which is to say not
mounted, the state every `files` share was in before the two halves agreed.
The same builder serves a pooled pod, a spawned one, and a hot attach into a
running one.
...
| `GATEWAY_KIND_OFF` | Refused by the **Operator**, not the agent: a grant of a kind whose switch is off on this deployment |
| `NODE_MOUNT_GATEWAY_INVALID_TARGET` | A target that is not a relative path of clean segments, three at most; never absolute, never walking |
...
Two of these carry more detail than the table has room for:

- **`GATEWAY_KIND_OFF`** — the Operator reads the same four switches
  (`DATALAYER_NODE_MOUNT_GATEWAY_CREDENTIALS`, `_BUCKETS`, `_LOCAL_BRIDGES`,
  `_REPOSITORIES`) so it never writes what the agent would answer
  `SECRET_REFUSED`; for an Environment's content it means the pool is
  declined and the launch spawns with the content.
- **`NODE_MOUNT_GATEWAY_INVALID_TARGET`** — the relative path is one segment
  for a Home Folder or a share, and `datasets/<name>` for an Environment's
  contents.
...
token, the relay URL and the **session key** all come from the Pod-owned
Secret, so a deployment detail is never right in two places. The session key
is what seals the frames end to end: the relay forwards what it cannot read,
and it can only not read them because both ends hold the key Contents minted
with the session. A mount started without one speaks plaintext at a client
speaking sealed frames, which surfaces not as a refusal but as
`frame header runs past the end of the frame` — neither end is told which the
other speaks. Like the token, it reaches the filesystem process in its
environment and never in the argv, and the relay URL is held to the same
...
**Whose mount it is.** A FUSE mount belongs to the user who made it. Inside a
sandbox, where this filesystem was written to run, the mounter *is* the reader
and that is right. Here root mounts and the sandbox's `datalayer` reads, in
another namespace, so the agent passes `--allow-other` (which needs
`user_allow_other` in `/etc/fuse.conf`, set in its image) and `--uid 1000
--gid 100`, so the files are reported as the sandbox's own rather than as
root's. Without the first the folder answers `Permission denied` to the person
who asked for it; without the second it is read-only to them whatever the
grant said.

The filesystem itself is fusepy, from `code-sandboxes`' **`[bridge]` extra**.
The image has to install `"./code-sandboxes[bridge]"`, not `./code-sandboxes`:
without the extra the package is present, the module is not, and every local
mount fails `FUSE_UNAVAILABLE: fusepy is not installed in the sandbox` — a
message written for a sandbox, arriving from a node agent whose `/dev/fuse`,
`SYS_ADMIN` and `fusermount` are all exactly as this page says they should be.

The inline `local.csi.datalayer.io` CSI volume that was the creation-time path
is retired (audit 82): a local mount is now a gateway process grant like any
other, and the chart defaults to gateway-only (`driver.gatewayOnly: true`), so
no `CSIDriver` or registrar is rendered.
...
the Secret the grant names. The intent is that **refreshing a mount becomes
refreshing a Secret** — no unmount, no remount, no open file handle broken
mid-read (see the status note below for the wiring that is still pending). Each
...
**In-place refresh is wired; verify against a real bucket when buckets go on.**
Each reconcile re-reads a live process grant's Secret and re-serves it to the
running mount (`NodeMountGateway.reconcile()` → `ProcessRouter.refresh()` →
`BucketProcesses.refresh()`), so a Secret the Operator replaces in place — an STS
session renewed under the same name — reaches the loopback endpoint within a
reconcile cycle and Mountpoint's SDK re-fetches live credentials rather than
403-ing when its first session expires. What is still unproven is `mount-s3`
itself against a real S3 bucket: that this credential URI is accepted and a
bucket actually mounts is the cluster-run check, and a mount that starts without
mounting anything is reported as a failure rather than as a mount.

### A static key, and a session

Two shapes of credential reach Mountpoint two ways. A **session** — what
the Operator mints for an attached bucket, refreshed in place before it
expires — is served to Mountpoint from the agent's loopback credential
endpoint (`AWS_CONTAINER_CREDENTIALS_FULL_URI`, one token per mount), so a
refresh is a new answer from the endpoint and never a remount. A **static
key** — what an Environment's bucket content names, copied by the Operator
into a Secret the pod owns, with the bucket's own region — is not a session:
it has no token and no expiration, and the container-credentials provider
handed one mounts and then answers nothing. So a static key travels in the
Mountpoint process's own environment, which no other process on the node
reads, and never in the argv. On the session path the agent strips any
`AWS_ACCESS_KEY_ID` its own process happened to carry before starting
Mountpoint, because the SDK reads the environment before it asks the
endpoint and a stray key would win over the session. Either way the agent
starts Mountpoint with `AWS_EC2_METADATA_DISABLED=true`: the SDK inside it
asks the EC2 instance-metadata service for whatever it was not told, and on
a node that is not an EC2 instance that is a probe that times out — 2.3 s
per mount, measured, against 0.18 s without — for an answer nothing needs.
...
### Rolling a new agent image

The agent is a DaemonSet with `maxUnavailable: 1`, so a new image goes one
node at a time, and an agent that restarts converges on the mounts it finds
in its pods' annotations rather than remaking them — a runtime on that node
keeps its folders through the roll. Bump the tag in the chart's
`driver.image` (and the image Makefile), push, then:

~~~bash
helm upgrade datalayer-node-mounts \
  $PLANE_HOME/etc/helm-private/charts/datalayer-node-mounts \
  -n datalayer-runtimes --reuse-values \
  --set driver.image=$DATALAYER_DOCKER_REGISTRY/node-mounts:<tag>
kubectl rollout status daemonset/datalayer-node-mounts -n datalayer-runtimes
clouder node-mounts verify
~~~

`--reuse-values` keeps the release's switches (the gateway, the claim, each
mount kind) and takes only the tag; `verify` should still answer `0 mount(s)
would not unmount` on every node once the roll is through. Never re-push a
tag the DaemonSet already runs: `imagePullPolicy: Always` would pull it on
the next restart, and nothing would say which agent a node is running.
...
**companion** uses all three: on pause it labels its own Pod `pod-status:
paused` and then deletes it. RBAC has no way to say "patch only your own Pod",
...
ls -la /home/datalayer/            # <handle> -> /mnt/datalayer/<handle>
mount | grep /mnt/datalayer     # nosuid, nodev; ro where the grant says ro
echo hello > /home/datalayer/$USER_HANDLE/gw-test.txt
...
Datashim upstream mounts several things — S3, NFS, H3. Here it mounted one:
`content_operator.py` created a `Dataset` for a `RuntimeContent` of `type: s3`
and skipped every other type, so no user Cloud Storage attachment went through
it either. It creates none now (audit 83) — the handler only caches and
validates the `RuntimeContent`, and the gateway mounts the bucket with
Mountpoint for S3.
...
**A pooled Pod is the only one that gets them this way.** A spawned (cold) Pod
carries the Environment's `git` and NFS contents in its spec — a checkout by an
init container, a `subPath` on the shared claim — because no gateway is serving
it; granting those again would mount each one twice. A pooled Pod renders
**none** of them and takes every content as a grant (audit 83). It defers on
purpose: a creation-time `subPath` would leave `/home/datalayer/<dir>` root-owned,
and the companion's link — made inside the sandbox as the runtime user — could
not add a sibling into it, so the bucket would reach `/mnt/datalayer` and never
the home path a user was promised. With nothing rendered, the link snippet owns
`/home/datalayer` and joins every content — the checkout, the share, the bucket —
to its promised path. (No Environment both carries a bucket and forces a cold
spawn, so a bucket is only ever a pooled-Pod `cloud-storage` grant.)

Two rules decide what a pooled Pod is given, and they are separate on purpose:

- **The Environment's contents are granted whether or not the launch brings
  folders of its own.** They are the Environment's promise, not the person's
  request. A launch with no Home Folders and no attachments still gets the
  tutorial repository its Environment declared — and the pool was only allowed
  for that launch *because* those contents could be granted, so not granting
  them would be the one outcome the check exists to prevent.
- **A launch that asked for no folders gets no home folder.** The caller's own
  ...
  `home-folder` capability is not one to mount a home folder on.

### It replaces it now (audit 83)

That was the plan, and it is **done**. The gateway's bucket path has been proven
against a real bucket on r1 — a Runtime Pod mounted its S3 datasets through the
gateway with Mountpoint for S3 — and the Operator no longer creates a `Dataset`
for an `s3` content. What remains is taking Datashim out of the clusters that
still run it (see the [Datashim page](/services/system/datashim/#teardown)).

Retiring Datashim removes three things at once:
...
### One thread, and how to tell it has stopped

The agent reconciles every pod on its node from a single thread. Two things
used to end that thread without ending the container: a pass that raised, and
a mount command that never returned — `umount` on a filesystem whose process
is gone waits for a server that will never answer. Either way the container
stayed `Running` with zero restarts, its health endpoint answered, its CSI
socket served, and every pod scheduled to that node from then on got no
mounts at all, silently.

Both are closed now: every mount command has a 20 s timeout and a timeout
counts as a failure, which `unmount` answers with its lazy forms; a pass that
raises is logged with its traceback and retried with backoff. And the agent's
snapshot carries the number that shows it either way —

~~~bash
clouder node-mounts status   # seconds_since_resync, failed_passes, reconciling
~~~

The leak check reads the same way. `clouder node-mounts verify` reports
**`stuck`** — paths that are mount points and should not be, re-checked
against the kernel each time it is asked, so the number clears itself when a
mount finally comes down — and mentions `counters.leaked` beside it, which
counts releases that hit any error since the agent started. They are
different questions: one unmount racing kubelet's teardown of the same
`emptyDir`, which comes down a moment later, moves the counter and not the
state. Reporting the counter as the state failed healthy nodes for the life
of a process, with nothing an operator could do to clear it.

`seconds_since_resync` climbing without bound is an agent that has stopped
working, whatever else says it is healthy. Nothing else here distinguishes
that from an agent with nothing to do.
...
umount -R /var/lib/kubelet/pods/POD_UID/volumes/kubernetes.io~empty-dir/node-mount-gateway
