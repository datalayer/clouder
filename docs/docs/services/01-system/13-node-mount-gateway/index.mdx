...
**Not yet verified against a real bucket, and in-place refresh is not yet
wired.** The runner, the credential endpoint and their `refresh` methods have
unit tests, but the reconcile loop does not yet re-read a changed Secret to
call them: a Secret replaced in place leaves the grant hash unchanged, so the
agent short-circuits, the mount keeps serving its original session, and it
returns 403 once that session expires. Turning buckets on for real means wiring
that re-read (`ProcessRouter.refresh` → `BucketProcesses.refresh` on a
process-kind grant whose Secret changed) and verifying `mount-s3` accepts this
credential URI on a real S3 bucket, as part of the cluster run; a mount that
starts without mounting anything is reported as a failure rather than as a mount.
