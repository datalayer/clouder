...
carries the Environment's contents in its spec, so granting them again would
mount each one twice. A pooled Pod was built before anyone knew which
Environment it would serve, which is why the grant is the only route it has.
