...
## Datalayer PostgreSQL DBOS Cluster

A second cluster on the same operator and the same operand image — not a
second operator. It lives in **`datalayer-dbos`**, not here: this namespace
holds the platform's own database, and what an engine needs to exist belongs
with that engine, so tearing DBOS down cannot come near the platform's data.
It holds the state of the durable execution service and of `agent-runtimes`:

| Database | Owner | Holds |
| --- | --- | --- |
| `dbos_durable` | `datalayer-durable` workers | Every workflow, every step's recorded result, every queue |
| `dbos_agents` | `agent-runtimes` | Its own durable state, reached through `DBOS_DATABASE_URL` injected by the Operator |
| `dbos_durable_dbos_sys` | DBOS itself | DBOS's **system** database, created at bootstrap |

`dbos_durable_dbos_sys` is named for the application database it backs. DBOS
normally creates its system database itself, where its role has permission to
create databases; this role does not, so the cluster creates it at bootstrap
instead. Without it, the workers launch, fail on `database
"dbos_durable_dbos_sys" does not exist`, and take no work at all.

```bash
plane postgresql-dbos-create      # cluster, its databases, and the workers' secret
plane postgresql-dbos-status
plane postgresql-dbos-terminate   # deletes every workflow in it; see Durable
```

Three instances, because losing one instance must lose no workflow: a durable
engine on a single-instance database is durable exactly until that instance
moves. `max_connections` is set above the sum of the durable queues'
concurrencies, with room for a rolling upgrade running two versions at once.

`postgresql-dbos-create` will not apply over an existing cluster. For the
memory cluster a re-apply is an update; this one holds work that is in
flight, so it points at `-status` and leaves the decision to you.

No command prints the connection string. `postgresql-dbos-create` assembles it
from the cluster's application secret and writes it into
`secret/datalayer-durable-dbos` in `datalayer-durable`, which is where the
chart reads it — so there is no `DBOS_DATABASE_URL` to export, and a password
never travels through `helm --set` or a shell history:

```bash
kubectl get secret datalayer-postgresql-dbos-app -n datalayer-dbos       # where it comes from
kubectl get secret datalayer-durable-dbos -n datalayer-durable           # where the workers read it
```

Unlike a cache, this cluster is the record of what work is in flight.
Restoring it late does not mean stale data — it means replaying workflows
that already finished. See [Durable](/services/system/durable/).
...
The same database holds what an agent runtime on Kubernetes promised over its
protocols, so it outlives the pod:

- The A2A tasks it issued, with their history and artifacts.
- Their conversation contexts.
- The runs still owed.
- Its ACP sessions, with each turn of their conversations.

They live in one table, `agent_runtime_protocol_state` (`kind`, `key`, `value`
JSONB, `updated_at`), which the runtime creates on first use; keys are scoped
by agent (`<agent>/<task>`) and by session (`<session>/<turn>`). A runtime
that starts finds the tasks a client polls and the sessions a client loads
again, and runs again the A2A tasks the previous pod left submitted or
working.

The runtime picks this database with the rule its memory backends use: on
Kubernetes (`KUBERNETES_SERVICE_HOST` set) and with the agent-memories variables
above, PostgreSQL; otherwise a SQLite file at
`AGENT_RUNTIMES_PROTOCOL_STATE_PATH` (default
`~/.datalayer/agent-runtimes/protocol-state.sqlite`), which does not survive a
pod. The driver is `psycopg`, a dependency of `agent-runtimes` itself.
