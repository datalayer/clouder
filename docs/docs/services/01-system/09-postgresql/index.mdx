...
second operator. It holds the state of the durable execution service and of
`agent-runtimes`:
...
Neither command prints the connection string. It is what `DBOS_DATABASE_URL`
is built from, and a password echoed into a terminal is a password in a
scrollback buffer:
