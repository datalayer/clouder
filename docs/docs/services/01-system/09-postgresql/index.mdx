...
| `dbos_durable_dbos_sys` | DBOS itself | DBOS's **system** database, named for the application one. It creates this itself where the role may create databases; this role may not, so the cluster creates it at bootstrap. Without it the workers launch, fail on `database "dbos_durable_dbos_sys" does not exist`, and take no work at all |
...
protocols, so it outlives the pod: the A2A tasks it issued (with their history
and artifacts), their conversation contexts, the runs still owed, and its ACP
sessions with each turn of their conversations. They live in one table,
`agent_runtime_protocol_state` (`kind`, `key`, `value` JSONB, `updated_at`),
which the runtime creates on first use; keys are scoped by agent (`<agent>/<task>`)
and by session (`<session>/<turn>`). A runtime that starts finds the tasks a
client polls and the sessions a client loads again, and runs again the A2A
tasks the previous pod left submitted or working.
