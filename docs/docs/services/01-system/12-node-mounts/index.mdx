...
They share an image, a chart and an upgrade. The gateway is the live service —
`nodeMountGateway.enabled` turns it on. The inline-CSI driver is **retired**
(audit 82): its delivery path is gone from the Operator, and the chart defaults
to gateway-only (`driver.gatewayOnly: true`), so the `CSIDriver` object and the
registrar sidecar are rendered **only** when `gatewayOnly` is set false. By
default the DaemonSet ships the gateway alone.

## The two services

| | [Node Mount Gateway](/services/system/node-mount-gateway/) | [Local CSI driver](/services/system/local-csi/) — **retired (audit 82)** |
| --- | --- | --- |
| **What it mounts** | Home folders, datasets, NFS exports, Git checkouts, buckets | Once: a folder on a user's own computer (that delivery is now a gateway grant) |
...
| **Is it CSI?** | **No**, deliberately: a PVC cannot attach to a running Pod, which is why the gateway exists | Yes, `local.csi.datalayer.io` — off by default |
| **Switch** | `nodeMountGateway.enabled`, off by default | `driver.gatewayOnly: false` to bring it back; off by default |

Among buckets, the gateway also delivers a folder on a user's own computer,
through the bridge relay.
...
| Piece | Where |
| --- | --- |
| **Image** | `plane/etc/dockerfiles/datalayer-node-mounts` → `datalayer/node-mounts:0.1.0` |
...
- the `csi` extra
- `code-sandboxes` (the bridge FUSE filesystem)
- `mount-s3` (Mountpoint for S3)
- `fuse3`
...
- a `DaemonSet` on the nodes labelled `role.datalayer.io/runtime=true`
  (`driver.nodeSelector`), with the privileged `driver` container,
  `/var/lib/kubelet/pods` mounted at the same path `Bidirectional`, and
  `/dev/fuse`. The upstream `csi-node-driver-registrar` sidecar and the
  `/var/lib/kubelet/plugins/local.csi.datalayer.io` socket mount are rendered
  only when `driver.gatewayOnly` is false (retired, off by default; audit 82);
- a `ServiceAccount` with a `ClusterRole` limited to `nodes: get` and
  ...
  namespaced `Role`;
- a `CSIDriver` `local.csi.datalayer.io`, for the driver — rendered only when
  `driver.gatewayOnly` is false (retired, off by default; audit 82)
...
| `DATALAYER_NODE_MOUNT_GATEWAY_CREDENTIALS`, `_BUCKETS`, `_LOCAL_BRIDGES`, `_REPOSITORIES` | the four mount-kind switches | All off unless set |
| `DATALAYER_LOCAL_CSI_RELAY_CIDR`, `_PORT` | `relay.cidr`, `relay.port` | The driver's bridge relay |
