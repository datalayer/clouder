...
| `/dashboards/` | GET, POST | JWT | The built-in MCP dashboards, then the account's own |
...
beside the platform's others. Five are built in, one per marketed area —
**calls and refusals**, **runs**, **sandboxes by provider**, **data access**,
**agents by organization** — and they are *generated* from a copy of the
gateway's catalogue (`datalayer_otel/dashboards.py`) that a contract test
holds equal to the gateway's `telemetry.CATALOGUE`. An instrument the
gateway adds without a panel fails a test rather than going undrawn; a
panel naming a label the gateway does not emit fails at import.

A dashboard is a list of panels. A panel is one metric, optionally split by
one of its labels, summarised one way: counters add up (`sum`), a gauge is
its latest reading (`last`), a duration is averaged (`avg`); `max` and
`count` exist. No percentile is promised — the metrics rows carry what the
exporter wrote per interval, not histogram buckets.
...
platform administrator asking for a built-in dashboard **with no
`account_uid`** reads that account rather than their personal one, where the
gateway has never written a point. The account is found in the store, from
