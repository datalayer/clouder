...
Not implemented, and therefore not queryable: `mcp.call.duration`,
`mcp.bindings`, `mcp.tasks`, `mcp.task.duration`, `sandbox.launch_seconds`,
`mcp.audit.write_failures`, `mcp.dependency.ready`, and every `durable.*`
instrument — those belong to the durable execution service, which is not
deployed.
