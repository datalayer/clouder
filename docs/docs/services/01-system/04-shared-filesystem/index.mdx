...
On AWS/EFS, `chown` is intentionally skipped in `shared-fs-prepare` to avoid noisy `Operation not permitted` errors from EFS access point ownership semantics.
