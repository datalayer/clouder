...
| [Mount gateway](/services/system/local-csi/#the-mount-gateway) | Whole claim, **once per node** in the `datalayer-local-csi` DaemonSet, at `gateway.sharedFilesystemMountPath`. It binds sub-paths of it into pods that are already running. |
