...
# shared-fs-pvc       Bound    pvc-916cfe94-f0dd-49d8-92c5-7b7ebef43160   100Gi      RWX            azure-nfs      <unset>                 70s
kubectl get pv | grep datalayer-runtimes
# pvc-b6a66bd4-39db-4649-9bf7-a71203a8e52d   100Gi          RWX            Delete           Bound      datalayer-runtimes/shared-fs-pvc       azure-nfs
```

A `shared-fs-prepare` Pod has been launched and is responsible to create the storage initial folders. The Ceph storage is mounted on `/mnt/sharedfs`.

```bash
kubectl logs shared-fs-prepare -n datalayer-runtimes
# + mkdir -p /mnt/sharedfs/home
# + chown 1000:100 /mnt/sharedfs/home
# + mkdir -p /mnt/sharedfs/public
# + chown 1000:100 /mnt/sharedfs/public
# + mkdir -p /mnt/sharedfs/datasets
# + chown 1000:100 /mnt/sharedfs/datasets
# + mkdir -p /mnt/sharedfs/tmp
# + chown 1000:100 /mnt/sharedfs/tmp
kubectl exec shared-fs-prepare -n datalayer-runtimes -it -- mount | grep /mnt/sharedfs
# 10.0.110.3:6789,10.0.202.233:6789,10.0.44.146:6789:/volumes/csi/csi-vol-a66fdf1a-7046-4bed-8d21-846d9b84f4df/23dde9c7-67f3-40a1-8329-49d93cd6a8de on /mnt/sharedfs type ceph (rw,relatime,name=csi-cephfs-node,secret=<hidden>,fsid=00000000-0000-0000-0000-000000000000,acl,mds_namespace=shared-filesystem)
kubectl exec shared-fs-prepare -n datalayer-runtimes -it -- ls /mnt/sharedfs
...
The `aws-efs` StorageClass must exist **before** running `plane up datalayer-shared-filesystem`.
The shared-filesystem chart creates PVCs only and expects a pre-provisioned EFS-backed StorageClass.

You also need:

- The AWS EFS CSI driver installed in the cluster.
- An existing EFS file system ID.

Create the StorageClass (example):
...
# + mkdir -p /mnt/sharedfs/tmp
# + chown 1000:100 /mnt/sharedfs/tmp
kubectl exec shared-fs-prepare -n datalayer-runtimes -it -- mount | grep ceph
# 10.0.110.3:6789,10.0.202.233:6789,10.0.44.146:6789:/volumes/csi/csi-vol-a66fdf1a-7046-4bed-8d21-846d9b84f4df/23dde9c7-67f3-40a1-8329-49d93cd6a8de on /mnt/sharedfs type ceph (rw,relatime,name=csi-cephfs-node,secret=<hidden>,fsid=00000000-0000-0000-0000-000000000000,acl,mds_namespace=shared-filesystem)
kubectl exec shared-fs-prepare -n datalayer-runtimes -it -- sh
kubectl delete pod shared-fs-prepare -n datalayer-runtimes
```
