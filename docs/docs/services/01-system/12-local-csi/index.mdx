...
inside a Datalayer Code Sandbox, through the Contents **local bridge**. This
page describes what ships in Clouder — the node driver, its image and its
chart — how it is installed, and what it expects from the two other pieces:
the bridge relay in Contents and the volume rendering in the Operator.
...
python -m pytest clouder/tests/test_csi_driver.py clouder/tests/test_csi_server.py \
  ...
  clouder/tests/test_local_csi_cli.py -q
~~~
