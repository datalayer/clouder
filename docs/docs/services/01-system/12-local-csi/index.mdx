...
in the block that turns `attachment_mounts` into pod volumes. For
`delivery == "local-bridge"` it renders the volume shown under
[The contract](#the-contract) and creates the `bridge-<bridge uid>` Secret.
The attachment travels on the pod as the `contents.datalayer.io/attachments`
annotation, so a rebuilt pod spec renders the same mounts. A `required: true`
attachment that cannot be mounted blocks readiness, the way a required Volume
attachment does.
...
an environment advertising `local-bridge-mount` in
`SandboxEnvironment.metadata` gets the mount from its adapter's `start()`;
Runtimes refuses a `local-bridge` attachment before launch anywhere else and
offers Synchronize instead.
