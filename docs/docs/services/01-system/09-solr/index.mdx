...
### Core Collections (22 during Contents migration)
...
The transitional inventory is 22 Core plus 8 Runtimes collections, 30 in
total. After Datasource migration removes the legacy `datasources` collection,
the final inventory is 21 Core, 8 Runtimes and 29 total. Always verify the
...
plane solr-init
# Select:
#   1) Core       - Platform collections (22 collections)
...
for COLLECTION in ai-agents api-keys contacts credits datasources evals events iam inbounds invites library notifications oauth-clients oauth-codes oauth-grants outbounds schedules secrets spaces success tool-approvals usage ray runtime-registrations runtime-checkpoints sandbox-snapshots contents content-objects content-operations content-audit
do
    echo
...
| `datalayer-backup-s3-core.yaml` | 22 Core collections | `datalayer-solr-collection-backups` |
