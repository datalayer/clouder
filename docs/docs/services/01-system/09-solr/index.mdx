...
### Core Collections (26 during Contents migration)
...
The transitional inventory is 26 Core plus 4 Runtimes collections, 30 in
total. After Datasource migration removes the legacy `datasources` collection,
the final inventory is 25 Core, 4 Runtimes and 29 total. Always verify the
arrays and manifests rather than relying on these numbers alone.

All four Contents collections — including `content-operations` — are Core
collections, and together they form one operational recovery scope. They are
read and written only by `datalayer-contents`, so initializing the Core set
alone gives the Contents API and worker every collection they need: a Core
initialization that omitted `content-operations` left the worker crashing at
startup with `Unknown collection`. The `contents` scope is available to
backup, status, task cleanup and restore commands; it does not create
...
plane solr-init
# Select:
#   1) Core       - Platform collections (26 collections)
#   2) Runtimes   - Runtimes collections (4 collections)
#   3) All        - All collections (30 collections)
#
...
for COLLECTION in ai-agents api-keys contacts content-audit content-objects content-operations contents credits datasources evals events iam inbounds invites library notifications oauth-clients oauth-codes oauth-grants outbounds schedules secrets spaces success tool-approvals usage ray runtime-registrations runtime-checkpoints sandbox-snapshots
do
    echo
...
Four backup definitions are available. Contents has a distinct resource name
so its coordinated schedule can run beside the full backup:

| File | Scope | CR Name |
|------|-------|---------|
| `datalayer-backup-s3.yaml` | All 30 transitional collections | `datalayer-solr-collection-backups` |
| `datalayer-backup-s3-core.yaml` | 26 Core collections | `datalayer-solr-collection-backups` |
| `datalayer-backup-s3-runtimes.yaml` | 4 Runtimes collections | `datalayer-solr-collection-backups` |
