...
The inventory is 21 Core plus 8 Runtimes collections, 29 in total. Always
...
plane solr-init
# Select:
#   1) Core       - Platform collections (21 collections)
#   2) Runtimes   - Runtimes collections (8 collections, Contents included)
#   3) All        - All collections (29 collections)
#
...
for COLLECTION in ai-agents api-keys contacts credits evals events iam inbounds invites library notifications oauth-clients oauth-codes oauth-grants outbounds schedules secrets spaces success tool-approvals usage ray runtime-registrations runtime-checkpoints sandbox-snapshots contents content-objects content-operations content-audit
do
    echo
...
| File | Scope | CR Name |
|------|-------|---------|
| `datalayer-backup-s3.yaml` | All 29 collections | `datalayer-solr-collection-backups` |
| `datalayer-backup-s3-core.yaml` | 21 Core collections | `datalayer-solr-collection-backups` |
