...
for COLLECTION in ai-agents contacts credits datasources events iam iam-tokens inbounds invites library notifications outbounds secrets spaces success tool-approvals usage runtime-checkpoints sandbox-snapshots
do
    echo
...
kubectl apply -f $PLANE_HOME/etc/specs/solr/backups/datalayer-backup-s3-core.yaml
# Runtimes collections only.
kubectl apply -f $PLANE_HOME/etc/specs/solr/backups/datalayer-backup-s3-runtimes.yaml
```
  </TabItem>
  <TabItem value="bash" label="Bash">
```bash
