...
### Core Collections (25 during Contents migration)
...
The transitional inventory is 25 Core plus 5 Runtimes collections, 30 in
total. After Datasource migration removes the legacy `datasources` collection,
the final inventory is 24 Core, 5 Runtimes and 29 total. Always verify the
arrays and manifests rather than relying on these numbers alone.

The four Contents collections form one operational recovery scope even though
`content-operations` belongs to the Runtimes initialization set. The
`contents` scope is available to backup, status, task cleanup and restore
...
plane solr-init
# Select:
#   1) Core       - Platform collections (25 collections)
#   2) Runtimes   - Runtimes collections (5 collections)
...
for COLLECTION in ai-agents api-keys contacts content-audit content-objects contents credits datasources evals events iam inbounds invites library notifications oauth-clients oauth-codes oauth-grants outbounds schedules secrets spaces success tool-approvals usage content-operations ray runtime-registrations runtime-checkpoints sandbox-snapshots
do
    echo
...
| `datalayer-backup-s3-core.yaml` | 25 Core collections | `datalayer-solr-collection-backups` |
| `datalayer-backup-s3-runtimes.yaml` | 5 Runtimes collections | `datalayer-solr-collection-backups` |
