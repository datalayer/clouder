...
### `platform_features_previewer` gates a rollout, not a secret

The benchmark navigation ships to previewers before it ships to everyone
(`PLAN_BENCHMARK.md` B1-14). The role is `platform_features_previewer`, granted
with `PUT /users/{id}/roles/{role}` like any other, and the web application
reads it from the account it already has — IAM has nothing new to enforce.

Three things about it are worth knowing when somebody reports that a page is
missing:
