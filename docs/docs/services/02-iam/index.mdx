...
`grant_types_supported` names three, and the token endpoint accepts exactly
those three — one list in `services/oauth_provider.py`, read by the OAuth
...
`client_credentials` and `jwt-bearer` are **not** served. Service agents
authenticate by a different route — see below.
