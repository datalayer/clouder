...
:::warning Only `tasks.open` has a reader today
`spend.credits` needs IAM's ledger and the two `sli.*` need the OTEL service.
...
- **Alert delivery beyond the app.** The rules exist, the evaluator runs on
  ...
  reader**: `spend.credits` needs IAM's ledger and the two `sli.*` need the
  OTEL service, so a rule naming one of those is counted `unreadable` every
  tick and never fires. That is the honest outcome — a reader answering zero
  would turn "nobody is watching this" into "this never happens" — but it is
