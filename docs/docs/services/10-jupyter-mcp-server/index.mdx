...
- **The organization and personal policy layers.** `/api/mcp/v1/policy`
  ...
  today. The layers *are* enforced now: a tool denylist or allowlist, a
  client allowlist and a per-minute cap refuse the call and name the layer
  that decided. What is missing is IAM serving them, so today every rule
  still reads "Datalayer default" and nothing narrows.
...
- **Audit forwarding.** `siem.py` delivers to a webhook or an object store,
  ...
  to.
- **Alert rules.** The evaluator is here; `McpAlertRule` is an IAM setting
  that does not exist yet, so there is nothing for it to evaluate.
