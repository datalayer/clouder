...
```
curl -s https://mcp.datalayer.run/api/mcp/readyz | jq .mode
"standalone"
```
...
| `DATALAYER_MCP_STANDALONE` | Run the MCP endpoint with none of the platform machinery behind it — see [Two modes](#two-modes-and-which-one-you-are-running). `true`/`1`/`yes`/`on`; anything else, including `false`, is off. It wins over `DATALAYER_MCP_STORE` below, and it requires `replicaCount: 1` |
...
`readyz`'s `mode` before chasing any of them.
:::
