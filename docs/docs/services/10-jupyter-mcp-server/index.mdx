...
`mcp-gateway` holds seven document families, told apart by `type_s`:
...
The last two are IAM's to write and the gateway never writes them; they live
...
`MCP_MAPPING_VERSION` is **6**. The mapping in `mcp_mappings.json` is
...
:::warning Half the conditions have a reader
`tasks.open` and `tasks.failed` do. `audit.write_failures`, `spend.credits`
and the two `sli.*` do not — they need IAM's ledger and the OTEL service —
so a rule naming one of those is counted `unreadable` on every tick and never
...
| --- | --- | --- |
| `audit-retention` | hour | Deletes `mcp-audit` rows past each organization's retention, never one an open export still holds. Registered only when Solr is configured — a deployment without it has no ledger to sweep, and the startup log says so |
...
- **The organizations retention sweeps.** The scheduler runs the sweep every
  hour and it finds nothing to do: the gateway cannot yet ask IAM for the
  organizations and their retention settings, so the list it walks is empty.
  Deliberately empty rather than defaulted — sweeping every organization
  under the platform's 90 days would delete an enterprise customer's audit
  four years early because this service could not read what they are
  entitled to.
- **Audit forwarding.** `siem.py` delivers to a webhook or an object store,
  signed and retried, and is not on the schedule: its destinations are an
  organization setting IAM does not serve, so there is nowhere to deliver
  to.
