...
| Field | Values |
| --- | --- |
| `condition` | `tasks.open`, `tasks.failed`, `audit.write_failures`, `spend.credits`, `sli.availability`, `sli.latency` |
...
:::warning Five of the six conditions have a reader
`tasks.open`, `tasks.failed`, `audit.write_failures` and `spend.credits` do.
The two `sli.*` do not — they need the OTEL service — so a rule naming one of
