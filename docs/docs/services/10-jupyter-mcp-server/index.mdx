...
`datalayer mcp bindings`, `datalayer mcp agents`, `datalayer mcp setup`,
`datalayer mcp audit` and `datalayer mcp tasks` work today. `datalayer mcp
trace`, `datalayer mcp metrics` and `datalayer mcp logs` report what the OTEL
service holds, which is everything the gateway exports.
