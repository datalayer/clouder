...
:::warning Four of the six conditions have a reader
`tasks.open`, `tasks.failed` and `audit.write_failures` do. `spend.credits`
and the two `sli.*` do not — they need IAM's ledger and the OTEL service — so
a rule naming one of those is counted `unreadable` on every tick and never
fires. Deliberately: a reader answering zero would turn "nobody is watching
this" into "this never happens" — but it is not the same as being watched.
Read the tick's counters, not the empty alerts list.
:::
