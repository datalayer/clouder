...
The ingress publishes exactly three prefixes on `mcp.datalayer.run`: `/mcp`,
`/.well-known/oauth-protected-resource` and `/api/mcp`. Nothing else on the
host is routed to this service.
...
Three Solr collections belong to this service, all in the **Core** set because
the gateway runs on the platform plane, and all three are one recovery unit —
the `mcp` scope. See [Solr](/services/system/solr/) and
...
| `mcp-audit` | nothing yet | The append-only audit trail. IAM already writes its own decisions in the `McpAuditEvent` shape, to a JSON-line logger by default (`DATALAYER_IAM_AUDIT_SINK_CLASS` names another sink); the gateway has no sink and serves no `/audit` route yet |
...
The gateway needs IAM, Spacer, Runtimes and Contents to be up, and the three
`mcp-*` collections to exist (`solr-init`). Build and push the image from the
Services repository:
...
Each probe is bounded at three seconds, so a hung dependency is reported as one
rather than hanging the kubelet's probe with it.

:::note The readiness probe still calls `healthz`

The chart's `jupyterMcpServer.readinessPath` is `/api/mcp/healthz`, so
Kubernetes acts on liveness for both probes and the dependency gating above is
only what you see when you query `readyz` yourself. Point the value at the
readiness route to make the cluster act on it:
