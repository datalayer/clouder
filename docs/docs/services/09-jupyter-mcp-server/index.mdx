...
- **The organization and personal policy layers.** `/api/mcp/v1/policy`
  ...
  today. The organization, team and personal layers are fetched from IAM and
  are empty until IAM serves them, so the page shows "Datalayer default"
  against every rule.
- **Policy enforcement beyond scopes.** The layers are *reported* and not yet
  *enforced*: a client allowlist, a DPoP requirement or a per-minute cap in an
  organization's policy is visible on the Policies page and does not stop a
  call.
