...
| An Environment whose contents mount deeper than one segment below the home (`/home/jovyan/datasets/<name>`) | **The pool**, now: a gateway target may be a path of up to three clean segments, so the Environment's contents are grants like the folders. Before that rule changed, such an Environment could not be served from the pool and every launch of it that mounted anything got its own Pod — the p95 gate's twenty seconds. |
