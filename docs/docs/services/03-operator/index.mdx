...
share a mount path or one is not an absolute path. A sandbox with attachments
always gets a pod of its own: a pre-warmed pool pod was built before the
request named anything to mount, so it is never handed one. Then, per
`delivery`:
