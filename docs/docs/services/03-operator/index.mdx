...
```bash
clouder azure helm-values --cluster r1
```

By default this creates:

```text
~/.clouder/r1/azure-operator-values.json
```
...
```text
~/.clouder/<current-context>/azure-operator-values.json
```

4. Optional: override the file path explicitly.

```bash
export DATALAYER_OPERATOR_AZURE_VALUES_FILE=/path/to/azure-operator-values.json
plane up datalayer-operator
```
...
helm upgrade \
  ...
  --create-namespace \
  --values ~/.clouder/r1/azure-operator-values.json
```
