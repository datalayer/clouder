...
:::note
CRIU support is now built into `clouder kubeadm setup` by default. The `--enable-criu` flag above is for the future `clouder kubernetes create` managed interface.
:::
...
# Scale workers
clouder kubernetes update-nodepool my-cluster workers --desired 5

# Delete cluster (removes all VMs and resources)
clouder kubernetes delete my-cluster
```
