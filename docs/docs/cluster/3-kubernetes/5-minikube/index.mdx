...
```bash
# Install minikube.
clouder minikube-stop
clouder minikube-rm
clouder minikube-install
```

```bash
# Start minikube.
clouder minikube-start
...
# https://minikube.sigs.k8s.io/tutorials/custom_cert_ingress
...
# https://minikube.sigs.k8s.io/handbook/addons/custom-images
...
exit
#
# https://minikube.sigs.k8s.io/tutorials/nginx_tcp_udp_ingress/
#
...
data:
  "8686": "datalayer/runtime-pools-runtime-pools-svc:8686"
EOF
#
...
127.0.0.1 solr-datalayer-solrcloud-0.solr-datalayer-solrcloud-headless.datalayer solr-datalayer-solrcloud-1.solr-datalayer-solrcloud-headless.datalayer solr-datalayer-solrcloud-2.solr-datalayer-solrcloud-headless.datalayer
...
plane up minikube k8s-dashboard
...
plane up minikube datalayer-simple
plane helm-ls
# View k8s resources.
  ...
  containers:
    - name: nfs-server
      ...
      securityContext:
        privileged: true
      args:
        # Pass the paths to share to the Plane image
        - /exports
EOF
```
...
```bash
# cassandra stargate https://stargate.io/stargate/1.0/quickstart/quick_start-rest.html
# 8081: Auth
# 8082: REST
# 8080: GraphQL
...
```bash
# cassandra stargate rest https://stargate.io/stargate/1.0/developers-guide/rest-using.html
...
# https://stargate.io/stargate/1.0/quickstart/quick_start-graphql.html
```
...
Deploy `Runtime Pools`.

```bash
eval $(minikube docker-env)
cd $DATALAYER_HOME/etc/docker/runtime-pools && \
  make build && \
  make push
plane up minikube runtime-pools
plane ls
# kubectl get svc -n datalayer | grep runtime-pools
# open $(minikube service runtime-pools-runtime-pools-svc --namespace datalayer --url)/api/runtime-pools
open http://minikube.local/api/jupyter-server/lab?token=60c1661cc408f978c309d04157af55c9588ff9557c9380e4fb50785750703da6
open http://minikube.local/api/jupyter-server/pool/default?token=60c1661cc408f978c309d04157af55c9588ff9557c9380e4fb50785750703da6
open https://minikube.local/api/jupyter-server/pool/default?token=60c1661cc408f978c309d04157af55c9588ff9557c9380e4fb50785750703da6
curl --insecure -I https://minikube.local/api/jupyter-server/api/jupyter-server?token=60c1661cc408f978c309d04157af55c9588ff9557c9380e4fb50785750703da6 | grep ookie
cd $DATALAYER_HOME/src && \
  make pf-runtime-pools
open http://localhost:8686/api/jupyter-server/lab?token=60c1661cc408f978c309d04157af55c9588ff9557c9380e4fb50785750703da6
plane down runtime-pools
```
...
```bash
eval $(minikube docker-env)
cd $DATALAYER_HOME/etc/docker/datatayer-iam && \
  make build && \
  make push
plane down datalayer-iam
plane up minikube datalayer-iam
kubectl get pods -n datalayer
kubectl describe certificate $DATALAYER_RUN_HOST-iam-cert-secret -n datalayer
open https://${DATALAYER_RUN_HOST}/api/auth/version
```

## Datalayer Library

Deplopy `Datalayer Library`.

```bash
eval $(minikube docker-env)
cd $DATALAYER_HOME/etc/docker/datalayer-library && \
  make build && \
  make push
plane down datalayer-library
plane up minikube datalayer-library
kubectl get pods -n datalayer
kubectl describe certificate $DATALAYER_RUN_HOST-library-cert-secret -n datalayer
