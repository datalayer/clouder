"""The kubeadm install hook and the `clouder node-mounts status` command."""
...
from ..cli import node_mounts as node_mounts_cli
from ..cli.kubeadm import node_mounts as hook
...
def chart_dir(tmp_path) -> Path:
    chart = tmp_path / "datalayer-node-mounts"
    (chart / "templates").mkdir(parents=True)
    (chart / "Chart.yaml").write_text("apiVersion: v2\nname: datalayer-node-mounts\nversion: 0.1.0\n")
...
def test_resolve_chart_dir_prefers_explicit_then_env(tmp_path, chart_dir, monkeypatch):
    ...
    target = plane_home / "etc" / "helm" / "charts" / "datalayer-node-mounts"
...
def test_install_script_embeds_the_chart_and_values(chart_dir):
    script = hook.build_node_mounts_install_script(
        chart_dir,
        image="registry.example.com/node-mounts:0.1.0",
        relay_host="r1.datalayer.run",
        relay_port=443,
        relay_cidr="203.0.113.0/24",
    )
    assert "helm upgrade --install datalayer-node-mounts" in script
    assert "--namespace datalayer-runtimes" in script
    assert "--set driver.image=registry.example.com/node-mounts:0.1.0" in script
    ...
    assert "datalayer-node-mounts/Chart.yaml" in names
    assert "datalayer-node-mounts/templates/csidriver.yaml" in names


def test_install_script_quotes_values(chart_dir):
    script = hook.build_node_mounts_install_script(chart_dir, relay_host="host; rm -rf /")
    assert "relay.host='host; rm -rf /'" in script
    assert "relay.cidr" not in script


def test_install_node_mounts_skips_without_chart(monkeypatch, tmp_path):
    monkeypatch.delenv("PLANE_HOME", raising=False)
    monkeypatch.delenv("DATALAYER_HOME", raising=False)
    monkeypatch.setattr(hook, "resolve_chart_dir", lambda explicit=None: None)
    assert hook.install_node_mounts(master={"ip": "1.2.3.4"}, resolved_user="ubuntu", key_path="/k") is False


def test_install_node_mounts_runs_the_script_on_the_master(monkeypatch, chart_dir):
    calls = []
    ...
    assert "helm upgrade --install datalayer-node-mounts" in calls[0][3]
...
def test_status_command_reports_nodes(monkeypatch):
    ...
    pods = {
        "items": [
            {
                "metadata": {"name": "datalayer-node-mounts-abcde"},
    ...
    def fake_ssh(ip, user, key_path, command, check=True):
        ...
        if "/gateway" in command:
            ...
            return _Result('{"error": "the Node Mount Gateway is not enabled on this node"}')
        raise AssertionError(command)

    monkeypatch.setattr(node_mounts_cli, "_infer_cluster_name", lambda cluster: "demo")
    monkeypatch.setattr(node_mounts_cli, "resolve_kubeadm_cloud_context", lambda cloud, cluster_name: ("aws", "acc"))
    monkeypatch.setattr(node_mounts_cli, "_resolve_cluster_vms", lambda name, cloud, context_id: {"master": {"ip": "1.2.3.4"}, "workers": []})
    monkeypatch.setattr(node_mounts_cli, "_resolve_ssh_key_for_cluster", lambda name: "/k")
    monkeypatch.setattr(node_mounts_cli, "_ssh_cmd", fake_ssh)

    result = runner.invoke(node_mounts_cli.node_mounts_app, ["status", "--json"])
    ...
    result = runner.invoke(node_mounts_cli.node_mounts_app, ["status"])
    assert result.exit_code == 0, result.output
...
def test_status_reports_the_mount_gateway_when_a_node_runs_one(monkeypatch):
    ...
    gateway = {
        "gateway_root": "/var/lib/datalayer/node-mount-gateway",
    ...
    assert result.exit_code == 0, result.output
    assert "Node Mount Gateway" in result.stdout
...
HEALTHY = {
    ...
    "containers[0].args": "[--endpoint,--node-mount-gateway]",
...
def _verify(monkeypatch, **overrides):
    ...
    result = runner.invoke(node_mounts_cli.node_mounts_app, ["verify", "--json"])
...
def test_a_gateway_that_is_simply_off_is_not_a_failure(monkeypatch):
    result, checks = _verify(
        monkeypatch,
        **{"/gateway": "", "DATALAYER_NODE_MOUNT_GATEWAY_ENABLED": "false"},
    )
