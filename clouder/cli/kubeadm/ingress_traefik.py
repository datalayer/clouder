...
def register(kubeadm_app: typer.Typer):
    ...
    def kubeadm_enable_ingress_traefik(
        ...
        """Enable ingress load balancing: deploy Traefik + Azure Load Balancer.

        Deploys Traefik in NodePort mode on the cluster via Helm, creates an
        Azure Load Balancer with a public IP, and wires LB rules (80/443) to
        the Traefik NodePorts via worker VM NICs.
        ...
        print(Panel(
            ...
            f"[bold]RG:[/bold]       {rg}\n\n"
            f"This will:\n"
            f"  1. Deploy/upgrade Traefik ingress controller (NodePort mode via Helm)\n"
        ...
        rc = _ssh_cmd_stream(master["ip"], resolved_user, key_path, _SCRIPT_INSTALL_TRAEFIK)
        if rc != 0:
