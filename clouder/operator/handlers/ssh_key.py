...
@kopf.timer('ssh-keys', interval=60, idle=100)
def create_ssh_key_handler(status, logger, **_):
    info = status['create_ssh_key_handler']
...
@kopf.on.create('ssh-keys')
def create_ssh_key_handler(spec, meta, logger, memo: kopf.Memo, **kwargs):
...
@kopf.on.delete('ssh-keys')
def delete_ssh_key_handler(logger, status, memo: kopf.Memo, **kwargs):
    logger.info(status)
    info = status['create_ssh_key_handler']
